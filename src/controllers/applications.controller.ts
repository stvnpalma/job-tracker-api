...
export const createApplication: RequestHandler = (req, res) => {
  ...
  const missingFields: string[] = []; // Fixed: TS Type
  ...
  return res.status(201).json(newApplication); // Fixed: Response variable
};
...
export const deleteApplication: RequestHandler = (req, res) => {
  ...
  if (index === -1) {
    // Fixed: Changed from res.json(404) to res.status(404)
    return res.status(404).json({ error: 'Application not found' });
  }
